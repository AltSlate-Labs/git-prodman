...
async function buildContext(root) {
    ...
    const epics = readEpics(root);
    if (epics.length > 0) {
        parts.push("## Epics\n" + epics.map((e) => `- **${e.id}**: ${e.title} (${e.status}, ${e.priority})${e.description ? `\n  ${e.description.slice(0, 200)}` : ""}`).join("\n"));
    }
    const specs = readSpecs(root);
    if (specs.length > 0) {
        parts.push("## Specs\n" + specs.map((s) => `- **${s.id}**: ${s.title} (${s.status})${s.epic ? ` [Epic: ${s.epic}]` : ""}`).join("\n"));
    }
    return parts.join("\n\n");
}
...
function getDashboardHTML(root) {
    const config = readConfig(root);
    const epics = readEpics(root);
    const specs = readSpecs(root);
            ...
            <div class="space-y-8">
              ...
              <div class="relative pl-8">
                ...
                <div class="bg-hover rounded-lg p-4 border border-base">
                  ...
                  ${m.epics && m.epics.length > 0 ? `<div class="flex flex-wrap gap-2">${m.epics.map((e) => `<span class="px-2 py-1 bg-gray-700/50 rounded text-xs font-mono text-cyan-400">${e}</span>`).join("")}</div>` : ''}
                </div>
              </div>`).join("")}
            </div>
          </div>
