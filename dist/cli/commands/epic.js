...
import { findProdmanRoot, readEpics, readEpic, writeEpic, generateId, } from "../../core/fs.js";
import { createEpic } from "../../core/schemas/epic.js";
...
epicCommand
    ...
    if (addDetails && !p.isCancel(addDetails)) {
        ...
        if (details.description) {
            epic.description = details.description;
        }
        if (details.criteria) {
            epic.acceptance_criteria = details.criteria
                .split(",")
                .map((c) => c.trim())
                .filter(Boolean);
        }
...
epicCommand
    ...
    // Display as table
    console.log();
    console.log(pc.bold(`${"ID".padEnd(10)} ${"Title".padEnd(35)} ${"Status".padEnd(12)} ${"Priority".padEnd(8)}`));
    console.log(pc.dim("─".repeat(70)));
    for (const epic of epics) {
        const statusColor = getStatusColor(epic.status);
        console.log(`${pc.cyan(epic.id.padEnd(10))} ${epic.title.slice(0, 33).padEnd(35)} ${statusColor(epic.status.padEnd(12))} ${epic.priority.padEnd(8)}`);
    }
    console.log();
});
// epic show
epicCommand
    ...
    console.log();
    console.log(pc.dim(`Created: ${epic.created_at} | Updated: ${epic.updated_at}`));
    console.log();
});
// epic update
epicCommand
