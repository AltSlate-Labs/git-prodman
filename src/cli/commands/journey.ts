...
import {
  createJourneyFrontmatter,
  createJourneyContent,
  type JourneyStep,
} from "../../core/schemas/journey.js";
import {
  JOURNEY_STATUSES,
  PRIORITIES,
  EMOTIONS,
...
function readTargetUsers(root: string): Array<{ id: string; name: string; priority: string }> {
...
journeyCommand
  ...
  .action(async (options) => {
    ...
    if (!goal) {
      ...
      if (p.isCancel(goal)) process.exit(0);
    }
    ...
    if (p.isCancel(priority)) process.exit(0);

    const s = p.spinner();
    s.start("Creating journey...");

    const id = generateId(root, "journey");
    const frontmatter = createJourneyFrontmatter(id, title as string, {
      persona: persona as string | null,
      goal: (goal as string) || "",
      priority: priority as typeof PRIORITIES[number],
    });
    const content = createJourneyContent(title as string);
...
journeyCommand
  ...
  .action((options) => {
    ...
    for (const journey of journeys) {
      ...
      console.log(
        `${pc.cyan(journey.id.padEnd(8))} ${statusColor(journey.status.padEnd(12))} ${journey.title} ${personaStr}`
      );
    }
    console.log();
  });
  ...
  .action((id, options) => {
    ...
    if (journey.steps.length > 0) {
      ...
      for (const step of journey.steps) {
        ...
        console.log(
          `  ${pc.dim(String(step.order).padStart(2, " "))} ${step.action} ${pc.dim(`[${step.touchpoint}]`)} ${emotionColor(step.emotion || "neutral")}`
        );
      }
    }
...
journeyCommand
  ...
  .action(async (id, options) => {
    ...
    if (options.addStep) {
      ...
      if (p.isCancel(touchpoint)) process.exit(0);
      ...
      if (p.isCancel(emotion)) process.exit(0);

      const newStep: JourneyStep = {
        order: updated.steps.length + 1,
        action: action as string,
        touchpoint: touchpoint as typeof TOUCHPOINTS[number],
        emotion: emotion as typeof EMOTIONS[number],
        pain_points: [],
        opportunities: [],
      };
