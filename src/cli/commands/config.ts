...
import { UserConfigSchema, type UserConfig } from "../../core/schemas/config.js";
...
configCommand
  ...
  .action(async () => {
    ...
    const answers = await p.group(
      {
        ...
        model: ({ results }) =>
          p.text({
            ...
            placeholder: getDefaultModel(results.provider as string),
          }),
        endpoint: ({ results }) =>
          results.provider === "ollama"
            ? p.text({
    ...
    const newConfig: UserConfig = {
      ai: {
        provider: answers.provider as "openai" | "anthropic" | "ollama",
        api_key: (answers.apiKey as string) || undefined,
      ...
      switch (config.ai.provider) {
        ...
        case "ollama": {
          const response = await fetch(
            `${config.ai.endpoint || "http://localhost:11434"}/api/tags`
          );
