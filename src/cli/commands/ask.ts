...
import { PRODMAN_FILES, PRODMAN_DIRS } from "../../core/constants.js";
...
export async function handleAsk(query: string): Promise<void> {
  ...
  if (!userConfig?.ai?.provider) {
    p.log.error(
      pc.red("No AI provider configured. Run: ") + pc.cyan("prodman config set")
    );
    process.exit(1);
  }
    ...
    parts.push(
      "## Epics\n" +
        epics
          .map(
            (e) =>
              `- **${e.id}**: ${e.title} (${e.status}, ${e.priority})${
