...
import {
  createDecisionFrontmatter,
  createDecisionContent,
} from "../../core/schemas/decision.js";
import { DECISION_STATUSES } from "../../core/constants.js";
...
decisionCommand
  ...
  .action(async (options) => {
    ...
    if (p.isCancel(deciders)) process.exit(0);
    ...
    const deciderList = deciders
      ? (deciders as string).split(",").map((d) => d.trim()).filter(Boolean)
      : [];

    const frontmatter = createDecisionFrontmatter(id, title as string, {
      deciders: deciderList,
    });
    const content = createDecisionContent(title as string);
...
decisionCommand
  ...
  .action((options) => {
    ...
    for (const dec of decisions) {
      ...
      console.log(
        `${pc.cyan(dec.id.padEnd(8))} ${statusColor(dec.status.padEnd(12))} ${dec.title}`
      );
    }
    console.log();
  });
