...
import {
  createSpecFrontmatter,
  createSpecContent,
  type Spec,
} from "../../core/schemas/spec.js";
...
specCommand
  ...
  .action(async (title: string, options) => {
    ...
    const spec: Spec = { ...frontmatter, content };
...
specCommand
  ...
  .action(async (options) => {
    ...
    console.log(
      pc.bold(
        `${"ID".padEnd(12)} ${"Title".padEnd(40)} ${"Status".padEnd(12)} ${"Epic".padEnd(10)}`
      )
    );
    ...
    for (const spec of specs) {
      ...
      console.log(
        `${pc.cyan(spec.id.padEnd(12))} ${spec.title.slice(0, 38).padEnd(40)} ${statusColor(
          spec.status.padEnd(12)
        )} ${(spec.epic || "-").padEnd(10)}`
      );
    }
    console.log();
...
specCommand
  ...
  .action(async (id: string, options) => {
    ...
    console.log();
    console.log(
      pc.dim(`Created: ${spec.created_at} | Updated: ${spec.updated_at}`)
    );
