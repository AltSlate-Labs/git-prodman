...
import { ISSUE_TYPES, ISSUE_STATUSES, PRIORITIES } from "../../core/constants.js";
...
issueCommand
  ...
  .action(async (options) => {
    ...
    const issue = createIssue(id, title as string, {
      type: type as any,
...
issueCommand
  ...
  .action((options) => {
    ...
    for (const issue of issues) {
      ...
      console.log(
        `${typeIcon} ${pc.cyan(issue.id.padEnd(8))} ${statusColor(issue.status.padEnd(12))} ${pc.yellow(issue.priority)} ${issue.title}`
      );
    }
    console.log();
  });
...
issueCommand
  ...
  .action((id) => {
    ...
    if (issue.epic) console.log(`${pc.dim("Epic:")} ${issue.epic}`);
    if (issue.assignee) console.log(`${pc.dim("Assignee:")} ${issue.assignee}`);
    if (issue.labels.length > 0) console.log(`${pc.dim("Labels:")} ${issue.labels.join(", ")}`);
